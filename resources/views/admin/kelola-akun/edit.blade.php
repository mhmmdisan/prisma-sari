...
<div class="form-container">
    ...
    <div class="card-form card border-0 rounded-4 shadow-sm">
        <div class="card-header bg-white rounded-top-4 py-3" style="border-bottom: 2px solid #ffc107;">
            <div class="d-flex align-items-center gap-2">
                ...
                <h5 class="mb-0 fw-bold" style="color: #1b5e20;">Form Edit Akun</h5>
            </div>
        </div>
        <div class="card-body p-4">
            <form method="POST" action="{{ route('admin.kelola-akun.update', $user->id) }}" id="formAkun" novalidate>
                @csrf
                @method('PUT')
    ...
    <!-- Form Reset Password -->
    <div class="card-reset-password card border-0 rounded-4 shadow-sm mt-4">
        <div class="card-header bg-white rounded-top-4 py-3" style="border-bottom: 2px solid #ffc107;">
            <div class="d-flex align-items-center gap-2">
                <div class="header-icon-reset">
                    <i class="bi bi-key"></i>
                </div>
                <h5 class="mb-0 fw-bold" style="color: #dc3545;">Reset Password</h5>
            </div>
        </div>
        <div class="card-body p-4">
            <form method="POST" action="{{ route('admin.kelola-akun.reset-password', $user->id) }}"
                id="formResetPassword" novalidate>
                @csrf
                <div class="row g-4">
                    <div class="col-md-6">
                        <label class="form-label fw-semibold" id="label_password">
                            <i class="bi bi-lock text-danger me-1"></i> Password Baru
                        </label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-semibold" id="label_password_confirmation">
                            <i class="bi bi-check-circle text-success me-1"></i> Konfirmasi Password Baru
                        </label>
...
<script>
// ============================================================
// VALIDASI & SUBMIT FORM EDIT AKUN (AJAX)
// ============================================================
document.getElementById('formAkun')?.addEventListener('submit', function(e) {
    ...
    fetch('{{ route("admin.kelola-akun.update", $user->id) }}', {
            method: 'POST',
            headers: {
...
document.getElementById('formResetPassword')?.addEventListener('submit', function(e) {
    ...
    // Reset error styling - tanpa optional chaining di assignment
    ...
    var errorPasswordConfirmation = document.getElementById('error_password_confirmation');

    if (passwordField) passwordField.classList.remove('error');
    if (passwordConfirmationField) passwordConfirmationField.classList.remove('error');
    if (errorPassword) errorPassword.style.display = 'none';
    if (errorPasswordConfirmation) errorPasswordConfirmation.style.display = 'none';
    ...
    if (password.value !== passwordConfirmation.value) {
        if (passwordConfirmationField) passwordConfirmationField.classList.add('error');
        if (errorPasswordConfirmation) errorPasswordConfirmation.style.display = 'block';
    ...
    var btnSubmit = document.querySelector('.btn-reset-password');
    var originalText = btnSubmit.innerHTML;

    btnSubmit.innerHTML = '<i class="bi bi-hourglass-split me-2"></i> Memproses...';
    btnSubmit.disabled = true;

    fetch('{{ route("admin.kelola-akun.reset-password", $user->id) }}', {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
...
document.getElementById('name')?.addEventListener('input', function() {
    ...
    var errorEl = document.getElementById('error_name');
    if (errorEl) errorEl.style.display = 'none';
});
document.getElementById('email')?.addEventListener('input', function() {
    this.classList.remove('error');
    var errorEl = document.getElementById('error_email');
    if (errorEl) errorEl.style.display = 'none';
});
document.getElementById('role')?.addEventListener('change', function() {
    this.classList.remove('error');
    var errorEl = document.getElementById('error_role');
    if (errorEl) errorEl.style.display = 'none';
});
document.getElementById('password')?.addEventListener('input', function() {
    this.classList.remove('error');
    var errorEl = document.getElementById('error_password');
    if (errorEl) errorEl.style.display = 'none';
});
document.getElementById('password_confirmation')?.addEventListener('input', function() {
    this.classList.remove('error');
    var errorEl = document.getElementById('error_password_confirmation');
    if (errorEl) errorEl.style.display = 'none';
});
